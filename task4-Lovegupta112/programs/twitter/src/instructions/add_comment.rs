...
pub struct AddCommentContext<'info> {
    ...
    #[account(init,payer=comment_author,space=8+Comment::INIT_SPACE,seeds=[COMMENT_SEED.as_bytes(),comment_author.key().as_ref(),{hash::hash(comment_content.as_bytes()).as_ref()},tweet.key().as_ref()],bump)]
