...
pub fn add_reaction(ctx: Context<AddReactionContext>, reaction: ReactionType) -> Result<()> {
    ...
    require!(tweet.likes.checked_add(1).is_some(),TwitterError::MaxLikesReached);
    require!(tweet.dislikes.checked_add(1).is_some(),TwitterError::MaxDislikesReached);
    tweet_reaction.reaction=match reaction {
        ReactionType::Like=>{
             tweet.likes +=1;
             ReactionType::Like
        },
...
pub struct AddReactionContext<'info> {
    ...
    #[account(init,payer=reaction_author,space=8+Reaction::INIT_SPACE,seeds=[TWEET_REACTION_SEED.as_bytes(),reaction_author.key().as_ref(),tweet.key().as_ref()],bump)]
